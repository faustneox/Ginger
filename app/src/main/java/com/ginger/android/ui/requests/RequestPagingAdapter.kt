...
/**
 * PagingDataAdapter for RequestEntity — mirrors RequestListAdapter but supports Paging 3.
 */
class RequestPagingAdapter : PagingDataAdapter<RequestEntity, RequestPagingAdapter.ViewHolder>(DIFF_CALLBACK) {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        if (item != null) holder.bind(item, showExtraDetails, listener)
    }
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int, payloads: MutableList<Any>) {
        if (payloads.isNotEmpty()) {
            ...
            if ("STATUS_CHANGED" == payload) {
                val item = getItem(position)
                if (item != null) holder.animateStatusChange(item)
                return
            }
        }
